...
typedef struct USEROPEN {
    ...
    int  diroff;                // 相应打开文件的目录项在父目录文件的dirno盘块中的目录项序号？块内序号？
...
void show_help(){
    ...
    printf("read\t\t文件名\t\t\t在打开文件状态下，读取该文件\n");
...
void my_format(){
    ...
    boot->root = 5;//根目录放在第5块
    boot->startblock = v_addr0 + BLOCKSIZE*5;//初始地址加5个盘块的大小
    ...
    root->first = 5;
    root->free = 1;
    root->length = 2 * sizeof(fcb);//根目录大小两个FCB？
...
int  my_write(int fd){
    ...
    while(fgets(Tmp,100,stdin)){
        ...
        {//把用户输入的数据100个为一组，转到texttmp上
              textTmp[i]   = Tmp[i];
              textTmp[i+1] = '\0';
        }
        if(strlen(Tmp) >= 3)
        {//若字符串长度大于3，截取最后3个进行校验
            Tmp2[0] = Tmp[strlen(Tmp)-4];
            Tmp2[1] = Tmp[strlen(Tmp)-3];
            Tmp2[2] = Tmp[strlen(Tmp)-2];//不应该减3，2，1么？
            Tmp2[3] = '\0';
        }
...
int  do_write(int fd, char *text, int len, char wstyle){
    ...
    //另外,如果游标很大,但是在寻找对应盘块的时候发现没有那个盘块,那么把缺少的盘块全都补上（为什么要补上？）
    while(off >= BLOCKSIZE){
        blockNum = fatPtr->id;
        if(blockNum == END){
            ...
            if(blockNum == END){
                ...
                //update FAT
                fatPtr->id = blockNum;
                fatPtr = (fat*)(v_addr0 + BLOCKSIZE + blockNum);//括号的位置
                fatPtr->id = END;
            }
        }
...
int  do_read(int fd, int len, char *text){
    ...
    //	④ 比较buf中从偏移量off开始的剩余字节数是否大于等于应读读的字节数len，如果是，则将从off开始的buf中的len长度的内容读入到text[]中；否则，将从off开始的buf中的剩余内容读入到text[]中；
    ...
    //fatPtr 当前盘块对应的fat
    fat* fatPtr = (fat *)(v_addr0+BLOCKSIZE) + blockNum;
    //读写指针转化为逻辑块块号及块内偏移量，与此同时查找fat表找到该逻辑块的磁盘块块号，从而找到在虚拟磁盘中的内存位置
    while(off >= BLOCKSIZE){
        off -= BLOCKSIZE;
        blockNum = fatPtr->id;
        if(blockNum == END){
        ...
        fatPtr = (fat*)(v_addr0 + BLOCKSIZE) + blockNum;//更新当前盘块对应的fat
    }
    //当前盘块号对应的盘块在虚拟磁盘中的实际位置
    unsigned char* blockPtr = v_addr0 + BLOCKSIZE*blockNum;
    //使用内存拷贝函数从源内存地址的起始位置开始拷贝若干个字节到目标内存地址中
    //把文件内容读进buf
    memcpy(buf, blockPtr, BLOCKSIZE);
    char *textPtr = text;
    //fcb* debug = (fcb*)text;
